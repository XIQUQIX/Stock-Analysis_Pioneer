...
class MACD:
    ...
    def get_macd(stock_code: str):
        try:
            ...
            stock_data = GLOBAL_STOCK_DATA
            df = pd.DataFrame(stock_data[stock_code])

            # 检查月DIF
            result = MACD.check_monthly_macd_DIF(stock_code, df)
            return result
...
class Mix:
    ...
    def get_MACD_2_condition(stock_code:str):
        ...
        # 查看DIF刚刚>0   (条件1)
        condition1 = day_MACD_data["DIF"].iloc[-1] > 0 and day_MACD_data["DIF"].iloc[-2] < 0

        # 零线上金叉   (条件2)
        condition2 = day_MACD_data["MACD_Golden"].iloc[-1] and day_MACD_data["DIF"].iloc[-1] > 0
