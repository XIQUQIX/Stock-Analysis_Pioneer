    ...
    def process_stock(stock_code:str, mode:str):
        ...
        try:
            ...
            # bol: 前次碰上轨，今日碰 中轨 or 下轨
                ...
                stock_code = calculate.Bol.get_bol(stock_code)
                return stock_code

            # bol: 前次碰上轨，今日碰 下轨
            elif mode == "bol_lower":
                stock_code = calculate.Bol.get_bol(stock_code, sub_mode="下")
                return stock_code
                       
            # 日MACD小于0的
            elif mode == "macd < 0":
                stock_code = calculate.MACD.get_daily_macd_below0(stock_code)
                return stock_code

            # 找出接近金叉的股票
            elif mode == "kdj_near_gold":
                stock_code = calculate.KDJ.get_near_golden_cross(stock_code)
                return stock_code

            # 找出J < K
            elif mode == "J_K":
                stock_code = calculate.KDJ.get_J_K(stock_code)
                return stock_code
